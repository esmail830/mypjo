...
export default function GeneralProductsScreen(){
  const [title, setTitle] = useState('');
  const [desc, setDesc] = useState('');
  const [list, setList] = useState([]);

  async function load(){ 
    const raw = await AsyncStorage.getItem('general_products');
  ...
  async function add(){
    ...
    setList(newList);
    await AsyncStorage.setItem('general_products', JSON.stringify(newList));
    setTitle(''); setDesc('');
  }

  return (
    <View style={styles.container}>
      <Text style={styles.title}>منتجات عامة</Text>
